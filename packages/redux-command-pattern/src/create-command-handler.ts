...
import {
  ...
  isIterable,
  isPromiseLike,
} from '@rhombus-toolkit/type-guards';
import {
  Await,
  DeepDictionary,
  DeepDictionaryItem,
...
) => CommandResult<TState, TEvents>; // | PromiseLike<CommandResult<TState>>

export type CommandFnAny = CommandFn<any, any, any>;

export type CommandGenerator<TState, TEvent extends StandardEventAny = StandardEventAny> = Generator<
  CommandResult<TState, TEvent>,
  CommandResult<TState, TEvent> | void,
  TState
>;
export type AsyncCommandGenerator<
  TState,
  TEvent extends StandardEventAny = StandardEventAny,
> = AsyncGenerator<CommandResult<TState, TEvent>, CommandResult<TState, TEvent> | void, TState>;
export type CommandResult<TState = any, TEvent extends StandardEventAny = StandardEventAny> = AnyTypeOf<
  TEvent,
  TState
>;

export type CommandMap<S = any, P = any, E extends StandardEventAny = StandardEventAny> = DeepDictionary<
  CommandFn<S, P, E>
>;
...
export type InferPayload<T extends CommandFnOrMap> = T extends Func<[any, ...infer TPayload], any>
  ? TPayload
...
export type CommandHandler<T extends DeepDictionaryItem<CommandFnAny>> = AsyncAction<
  [commandObject: StandardCommand<InferPayload<T>>, signal?: AbortSignal]
>;

export function createCommandHandler<T extends DeepDictionary<CommandFnAny>>(
  store: Store,
  implementation: T,
): CommandHandler<T> {
...
async function handle(result: CommandResult, store: Store, signal?: AbortSignal): Promise<void> {
  ...
  if (isGenerator(result) || isAsyncGenerator(result)) {
    // this MUST happen before the 'isIterable()' check
...
function handleThunk(
  thunk: Thunk<CommandResult<any, StandardEventAny>>,
  store: Store,
  signal?: AbortSignal,
): Promise<void> {
...
async function handlePromiseLike(
  value: PromiseLike<CommandResult<any, any>>,
  store: Store,
  signal?: AbortSignal,
): Promise<void> {
...
function handleObservable(
  obs: Observable<CommandResult<any, any>>,
  store: Store,
  signal?: AbortSignal,
): Promise<void> {
...
async function handleGenerator(
  iter: CommandGenerator<any, any> | AsyncCommandGenerator<any, any>,
  store: Store,
  signal?: AbortSignal,
): Promise<void> {
...
async function handleIterable(
  value: Iterable<CommandResult<any, any>> | AsyncIterable<CommandResult<any, any>>,
  store: Store,
  signal?: AbortSignal,
): Promise<void> {
