...
import {
  ...
  CommandHandler,
  CommandMap,
  InferPayload as CommandPayload,
} from './create-command-handler';
...
type CommandCreatorOrMap<
  T extends DeepDictionaryItem<CommandFnAny>,
  OverrideReturn = never,
> = T extends CommandFnAny
  ? Func<
      [...payload: restify<CommandPayload<T>>],
      OverrideReturn extends never ? StandardCommand<CommandPayload<T>> : OverrideReturn
    >
  : T extends Record<string, any>
  ? CommandCreatorMap<T, OverrideReturn>
  : never;
...
} = function _getCommands<T extends CommandMap>(
  invoker?: CommandHandler<T>,
  type?: string,
): CommandCreatorMap<T> {
