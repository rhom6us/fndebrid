...
export function createReducer<T extends DeepDictionary<ReducerFnAny>>(
  reducers: T,
): ReduxReducer</*InferState<T>*/ any, EventTypes<T>> {
  const flatMap: any = flattenMap(reducers);
  return function rootReducerfn(state, { type, payload }) {
    if (state === undefined) {
      throw new TypeError(
        'redux-command-pattern does not support undefined state. Please preload with an initial state',
      );
    }
    if (!(type in flatMap)) {
