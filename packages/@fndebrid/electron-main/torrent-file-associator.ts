...
async function setAssReg(ns: string, ext: string, execPath: string) {
  ...
  iconPath.withDefault().set('REG_SZ', path.resolve(__static, 'android-chrome-192x192.png'));
  const commandPath = await nsPath.withPath('Shell/open/command').ensure();
  commandPath.withDefault().set('REG_SZ', `${execPath} "%1"`);
  const extPath = await Registry.HKCU.withPath('Software/Classes')
    .withPath(`.${ext.replace(/^\./, '')}`)
    .ensure();
  extPath.withDefault().set('REG_SZ', ns);
