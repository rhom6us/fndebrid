...
export abstract class Associator {
  ...
  public associate(associate: boolean = true): void {
    if (!associate) return this.disassociate();
    if (this.isAssociated) return;
    this._associate();
    if (!this.isAssociated) throw new OperationFailedError(`${this.constructor.name} failed to associate`);
  }
  public disassociate(): void {
    if (!this.isAssociated) return;
    this._disassociate();
    if (this.isAssociated) throw new OperationFailedError(`${this.constructor.name} failed to disassociate`);
  }
}
