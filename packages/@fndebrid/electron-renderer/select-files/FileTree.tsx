...
import {
  TreeNode as FnTreeNode,
  FolderNode as FnFolderNode,
  FileNode as FnFileNode,
  TreeNodeId,
  FolderNode,
} from './TreeNode';
...
export const FileTree: React.FC<IOwnProps> = ({files, onSelectionChanged}) => {
  ...
  function handleNodeClick(node: FnTreeNode<File>, _nodePath: number[], e: React.MouseEvent<HTMLElement>) {
    ...
    unselectedFolders && unselectedFolders.filter(folder => folder.files.every(isSelected)).forEach(selectNode);
    selectedFolders && selectedFolders.filter(folder => !folder.files.every(isSelected)).forEach(unselectNode);
