...
export function Dialog ({ children, title, onClose }: PropsWithChildren<{title:string, onClose: Function}>)  {
  return (
    <HeightBpDialog onClose={onClose as any} backdropProps={{hidden: true}} autoFocus={true} usePortal={false} hasBackdrop={false} isOpen={true} isCloseButtonShown={true} title={title}>
      {children}
    </HeightBpDialog>
  )
}
