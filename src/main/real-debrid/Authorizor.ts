...
export class Authorizor {
    ...
    async credentials({ client_id, device_code }: Pick<AuthorizatioInfo, 'client_id'> & {
        device_code: string;
    }) {
    ...
    async token({ client_id, client_secret, device_code }: Pick<AuthorizatioInfo, 'client_id' | 'client_secret'> & {
        device_code: string;
    }): Promise<AuthorizatioInfo> {
